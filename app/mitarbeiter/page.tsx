      ...
      <section className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-white/[0.08] via-white/[0.035] to-black/20 p-6 shadow-2xl shadow-black/30 lg:p-8">
        ...
        <div className="relative z-10 flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
          ...
          <div className="grid grid-cols-2 gap-2 rounded-3xl border border-white/10 bg-black/25 p-2 text-center backdrop-blur-xl sm:p-3 md:grid-cols-4">
            <HeroMini
              label="Team"
              value={String(mitarbeiter.length).padStart(2, "0")}
            />
            <HeroMini
              label="Aktiv"
              value={String(aktive).padStart(2, "0")}
              green={aktive > 0}
            />
            <HeroMini
              label="Probe"
              value={String(probezeit).padStart(2, "0")}
              blue={probezeit > 0}
            />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-4 grid grid-cols-3 gap-2">
                        <MiniStat
                          label="Woche"
                          value={formatStunden(person.wochenstunden)}
                        />
                        <MiniStat
                          label="Ferien"
                          value={`${person.ferienwochen || 4} W`}
                        />
                        <MiniStat
                          label="Status"
                          value={
                ...
                >
                  ...
                  <div className="space-y-5">
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                      <InfoCard
                        label="Wochenstunden"
                        value={formatStunden(selected.wochenstunden)}
                      />
                      <InfoCard
                        label="Ferienwochen"
...
function MiniStat({ label, value }: { label: string; value: string | number }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-3">
      ...
      <div className="mt-2 truncate text-sm font-black text-sky-100">
        {value}
      </div>
    </div>
  );
...
function InfoCard({
  ...
  return (
    <div className="rounded-[1.5rem] border border-white/10 bg-black/25 p-4 transition-colors hover:border-sky-300/25 hover:bg-sky-300/5">
      ...
      <div className={`mt-3 break-words text-2xl font-black ${color}`}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
      ...
      <div className="mt-3 overflow-hidden rounded-full bg-black/40">
        <div
          className="h-2 rounded-full bg-gradient-to-r from-sky-200 to-green-300"
          style={{ width: `${value}%` }}
        />
      </div>
    </div>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="flex min-h-[160px] items-center p-6 text-sm font-bold text-white/55">
      {text}
    </div>
  );
}
